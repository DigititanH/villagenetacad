# PHASE 5 - Shared cart (live catalogue + server cart + website PayFast)
...
$zipPath = Join-Path $env:TEMP "vna-phase5.zip"
$extractRoot = Join-Path $env:TEMP "vna-phase5"

Write-Host "Downloading Phase 5 from DigititanH/$branch ..." -ForegroundColor Cyan
...
Copy-Item (Join-Path $pack "docs\30-PHASE5-SHARED-CART.md") $docsDest -Force -ErrorAction SilentlyContinue
...
$checks = @(
  ...
  "presentation\customer\cart_screen.dart",
...
Write-Host ""
Write-Host "SUCCESS - Phase 5 lib is on this machine." -ForegroundColor Green
Write-Host ""
Write-Host "Run (live shared cart + website PayFast):" -ForegroundColor Yellow
Write-Host "  cd mobile"
Write-Host "  flutter pub get"
Write-Host "  flutter run --no-dds --dart-define=API_BASE_URL=https://villagenetacad.co.za"
Write-Host ""
Write-Host "UAT: sign in -> Store -> add item -> Complete on website -> My orders"
Write-Host "See docs\30-PHASE5-SHARED-CART.md"
