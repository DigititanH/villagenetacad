...
class Payfast
{
    ...
    public static function getNotifyUrl(): string
    {
        ...
        if ($custom !== '') {
            return rtrim(self::normalizeNotifyUrl($custom), '/');
        }
        return rtrim(self::getApiBaseUrl(), '/') . '/api/payfast/notify';
    }

    /**
     * Rewrite legacy Afrihost/PayFast paths to the live API route.
     * e.g. https://www.villagenetacad.co.za/payfast/notify.php
     *   -> https://villagenetacad.co.za/api/payfast/notify (via API_URL when set)
     */
    public static function normalizeNotifyUrl(string $url): string
    {
        ...
        $api = rtrim(self::getApiBaseUrl(), '/');
        if ($api !== '' && !self::isNotifyUrlLocal($api)) {
            return $api . '/api/payfast/notify';
        }

        $parts = parse_url($url);
        $scheme = $parts['scheme'] ?? 'https';
        $host = $parts['host'] ?? 'villagenetacad.co.za';
        // Prefer apex host when www is used on legacy URLs
