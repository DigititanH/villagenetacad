...
class Payfast
{
    ...
    /**
     * @param bool $usePostedOrder  ITN must use POST field order; outbound forms use FIELD_ORDER.
     * @param bool $useUrlEncode    Official ITN sample uses urlencode; forms often use pfEncode.
     */
    public static function buildSignatureString(
        array $data,
        ?string $passphrase = null,
        bool $usePostedOrder = false,
        ...
        $encode = $useUrlEncode
            ? static fn (string $v): string => urlencode($v)
            : [self::class, 'pfEncode'];

        $parts = [];
        foreach ($orderedKeys as $key) {
            $parts[] = $key . '=' . $encode($payload[$key]);
        }
    ...
    public static function verifyItnSignature(array $data): bool
    {
        ...
        $variants = [
            [true, true],   // posted order + urlencode (official ITN sample)
            [true, false],  // posted order + pfEncode
