...
class AppEmails
{
    ...
    public static function orderStatusCustomer(
        ...
        if ($st === 'processing') {
            ...
            $extra = '<p>We hope you enjoy your purchase. You can leave a review in the app under <strong>My orders</strong>. Returns are available for 7 days after delivery if the item is unused.</p>';
        } elseif ($st === 'cancelled') {
        ...
        } elseif ($st === 'return_requested') {
            $extra = '<p>We have logged your return request and will be in touch shortly.</p>';
        }
    ...
    /** Customer confirmation after requesting a return. */
    public static function returnRequestedCustomer(array $order, string $reason): void
    {
        $userId = (int) ($order['user_id'] ?? 0);
        $user = Database::queryGet(
            'SELECT r.name, l.email FROM registrations r
        ...
        self::sendApp(
            ...
            <p><strong>Your reason:</strong><br>{$safeReason}</p>
            <p>Our team will review your request and contact you with next steps. You can track the order under <strong>Profile → My orders</strong> in the app.</p>
            <p>— Village NetAcad · Powered by Digititan</p>"
        );
    }

    /** Ops notice when a customer requests a return. */
    public static function returnRequestedOps(array $order, string $reason): void
    {
        $orderId = (int) ($order['id'] ?? 0);
        $userId = (int) ($order['user_id'] ?? 0);
        $user = Database::queryGet(
        ...
        $name = htmlspecialchars((string) ($user['name'] ?? 'Customer'));
        ...
        Mailer::send([
            'to' => $to,
            'subject' => "Return requested — order #{$orderId}",
            'html' => "<h2>Return request</h2>
            ...
            <p>Please review in Ops Admin and follow the returns process.</p>
            <p>— Village NetAcad app</p>",
            'channel' => 'app',
        ]);
    }
