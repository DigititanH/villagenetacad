...
/**
 * Real SMTP send (cPanel mailbox preferred; Gmail App Password also supported).
 * Does NOT use PHP mail() — Afrihost often needs authenticated SMTP.
 * If SMTP_USER / SMTP_PASS missing: logs only (no silent fake success via mail()).
 */
class Mailer
{
    public static function send(array $opts): void
    {
        ...
        // App passwords / mailbox passwords sometimes pasted with spaces.
        $smtpPass = preg_replace('/\s+/', '', (string) (Env::get('SMTP_PASS') ?? ''));
        $from = $smtpUser !== '' ? $smtpUser : (Site::email() ?: 'noreply@villagenetacad.co.za');
        ...
        try {
            // Default host = cPanel/local mail — Gmail (smtp.gmail.com) is often blocked on Afrihost.
            self::smtpSend([
     ...
     *
     * @param array{to:string,subject?:string,html?:string} $opts
     */
    public static function sendOrFail(array $opts): void
    {
        ...
        if ($smtpUser === '' || $smtpPass === '') {
            throw new RuntimeException('SMTP_USER / SMTP_PASS not set in .env');
        }
        if ($to === '') {
